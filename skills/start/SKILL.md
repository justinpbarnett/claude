...
If a `.env` file is expected but missing:
- Check for `.env.example` or `.env.sample` and prompt the user to copy and configure it
- Check for `docker-compose.yml` -- if present, background services (database, redis, etc.) may need to start first via `docker compose up -d`
...
2. **Environment** -- Check for .env files and background services
3. **Start** -- Run the discovered command
4. **Confirm** -- Verify the server starts without errors
...
<If: startup fails due to outdated or incompatible dependencies>
<Then: use the `deps` agent to audit and resolve dependency issues before retrying.>
