...
Run the validation checklist in `references/validation-checklist.md` against the skill. Fix every violation before proceeding.

### Step 9: Write the Files

Create all files in the appropriate skills directory:
- **Project skill** -- `.claude/skills/{skill-name}/` (default)
- **Global skill** -- `~/.claude/skills/{skill-name}/` (when the user requests a global skill or the skill is project-agnostic)
...
<If: writing the description field>
<Then: invest disproportionate effort here. The description is the most important part of a skill -- a poorly triggered skill is worse than no skill. Include natural trigger phrases, negative triggers, and paraphrases.>
