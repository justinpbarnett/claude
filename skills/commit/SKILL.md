...
- **description**: present tense, lowercase, no period at the end. Keep the full line (type + colon + space + description) under 72 characters.
...
- Test additions/changes accompany the code they test in the same commit, unless they are standalone test improvements
...
- If a single file contains changes for different logical commits, use `git add -p <file>` to stage specific hunks
...
- Verify each commit message is under 72 characters (type + colon + space + description)
...
2. feat: add user model and auth route — src/models/user.ts, src/routes/auth.ts, tests/auth.test.ts
3. chore: add new build recipes — Makefile
```
