...
Return the path to the documentation file and a one-line summary of what was documented.

## Workflow

1. **Collect** -- Gather feature name and spec_path from user input
...
6. **Report** -- Return the documentation file path and summary
...
<If: screenshots are provided or a screenshots directory exists>
<Then: copy images to docs/assets/ and reference them in the documentation. If no screenshots, omit the Screenshots section.>
...
<If: changes include new or modified API endpoints>
<Then: delegate to the `api-docs` agent to generate detailed endpoint documentation (routes, parameters, response schemas) for inclusion in the feature docs.>
